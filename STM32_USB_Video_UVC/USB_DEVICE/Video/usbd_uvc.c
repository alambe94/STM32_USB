...
__ALIGN_BEGIN struct usbd_uvc_cfg USBD_UVC_CfgFSDesc __ALIGN_END = {
    ...
    .uvc_vc_input_terminal = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_input_terminal), // Descriptor size
        .bDescriptorType = UVC_CS_INTERFACE,                             // 36 (INTERFACE)
    ...
    .uvc_vc_processing_unit = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_processing_unit), // Descriptor size
    ...
    .uvc_vc_xu_lep_agc = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_agc), // Descriptor size
        .bDescriptorType = 0x24,                                     // Class specific interface desc type
    ...
    /* Extension Unit Descriptor */
    .uvc_vc_xu_lep_oem = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_oem), // Descriptor size
        .bDescriptorType = 0x24,                                     // Class specific interface desc type
        .bDescriptorSubType = 0x06,                                  // Extension Unit Descriptor type
        .bUnitID = VC_CONTROL_XU_LEP_OEM_ID,                         // ID of this terminal
        .guidExtensionCode = {                                       // 16 byte GUID
                              'p', 't', '1', '-', 'l', 'e', 'p', '-', 'o', 'e', 'm', '-', '0', '0', '0', '0'},
    ...
    /* Extension Unit Descriptor */
    .uvc_vc_xu_lep_rad = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_rad), // Descriptor size
        .bDescriptorType = 0x24,                                     // Class specific interface desc type
        .bDescriptorSubType = 0x06,                                  // Extension Unit Descriptor type
        .bUnitID = VC_CONTROL_XU_LEP_RAD_ID,                         // ID of this terminal
        .guidExtensionCode = {                                       // 16 byte GUID
                              'p', 't', '1', '-', 'l', 'e', 'p', '-', 'r', 'a', 'd', '-', '0', '0', '0', '0'},
    ...
    /* Extension Unit Descriptor */
    .uvc_vc_xu_lep_rad_2 = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_rad_2), // Descriptor size
        .bDescriptorType = 0x24,                                       // Class specific interface desc type
        .bDescriptorSubType = 0x06,                                    // Extension Unit Descriptor type
        .bUnitID = VC_CONTROL_XU_LEP_RAD_2_ID,                         // ID of this terminal
        .guidExtensionCode = {                                         // 16 byte GUID
                              'p', 't', '1', '-', 'l', 'e', 'p', '-', 'r', 'a', 'd', '2', '0', '0', '0', '0'},
    ...
    /* Extension Unit Descriptor */
    .uvc_vc_xu_lep_sys = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_sys), // Descriptor size
        .bDescriptorType = 0x24,                                     // Class specific interface desc type
        .bDescriptorSubType = 0x06,                                  // Extension Unit Descriptor type
        .bUnitID = VC_CONTROL_XU_LEP_SYS_ID,                         // ID of this terminal
        .guidExtensionCode = {                                       // 16 byte GUID
                              'p', 't', '1', '-', 'l', 'e', 'p', '-', 's', 'y', 's', '-', '0', '0', '0', '0'},
    ...
    /* Extension Unit Descriptor */
    .uvc_vc_xu_lep_vid = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_vid), // Descriptor size
        .bDescriptorType = 0x24,                                     // Class specific interface desc type
        .bDescriptorSubType = 0x06,                                  // Extension Unit Descriptor type
        .bUnitID = VC_CONTROL_XU_LEP_VID_ID,                         // ID of this terminal
        .guidExtensionCode = {                                       // 16 byte GUID
                              'p', 't', '1', '-', 'l', 'e', 'p', '-', 'v', 'i', 'd', '-', '0', '0', '0', '0'},
    ...
    /* Extension Unit Descriptor */
    .uvc_vc_xu_lep_cust = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_lep_cust), // Descriptor size
        .bDescriptorType = 0x24,                                      // Class specific interface desc type
        .bDescriptorSubType = 0x06,                                   // Extension Unit Descriptor type
        .bUnitID = VC_CONTROL_XU_LEP_CUST_ID,                         // ID of this terminal
        .guidExtensionCode = {                                        // 16 byte GUID
                              'p', 't', '1', '-', 'l', 'e', 'p', '-', 'c', 'u', 's', 't', '0', '0', '0', '0'},
    ...
    .uvc_vs_input_header_desc = {
        .bLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vs_input_header_desc),
        .bDescriptorType = UVC_CS_INTERFACE,       // 36 (INTERFACE)
        .bDescriptorSubType = UVC_VS_INPUT_HEADER, // 1 (INPUT_HEADER)
        .bNumFormats = VS_NUM_FORMATS,             // number of format descriptors
        .wTotalLength = SIZEOF_M(struct usbd_uvc_cfg, uvc_vs_input_header_desc) + SIZEOF_M(struct usbd_uvc_cfg, uvc_vs_frames_formats_desc),
  ...
  case USB_REQ_TYPE_CLASS:
    if (req->wLength)
    {
      ...
      if (req->bmRequest & USB_REQ_READ_MASK)
      {
        ...
        {
          ret = ((USBD_UVC_ItfTypeDef *)pdev->pUserData)->VS_CtrlGet(req->bRequest, (uint8_t *)huvc->data, req->wLength, req->wIndex, req->wValue);
        }
        else
        {
          ret = ((USBD_UVC_ItfTypeDef *)pdev->pUserData)->ControlGet(entity_id, req->bRequest, (uint8_t *)huvc->data, req->wLength, req->wIndex, req->wValue);
        }

        if (ret == USBD_OK)
        {
          USBD_CtlSendData(pdev, (uint8_t *)huvc->data, req->wLength);
        }
        else
        {
      ...
      else
      {
        ...
        huvc->CmdIndex = req->wIndex;
        huvc->CmdValue = req->wValue;
        USBD_CtlPrepareRx(pdev, (uint8_t *)huvc->data, req->wLength);
      }
    }
    else
    {
...
static uint8_t USBD_UVC_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  {
    uint8_t address = (huvc->CmdIndex >> 0) & 0xff;
    uint8_t entity_id = (huvc->CmdIndex >> 8) & 0xff;

    if (address == USB_UVC_VSIF_NUM)
    {
      ret = ((USBD_UVC_ItfTypeDef *)pdev->pUserData)->VS_CtrlSet(huvc->CmdOpCode, (uint8_t *)huvc->data, huvc->CmdLength, huvc->CmdIndex, huvc->CmdValue);
    }
    else
    {
      ret = ((USBD_UVC_ItfTypeDef *)pdev->pUserData)->ControlSet(entity_id, huvc->CmdOpCode, (uint8_t *)huvc->data, huvc->CmdLength, huvc->CmdIndex, huvc->CmdValue);
    }
